...
export default function CreatePositionPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.supplier_id) {
      toast({
        title: "Eksik bilgi",
        description: "Lütfen bir tedarikçi / nakliyeci seçin.",
        variant: "destructive",
      });
      return;
    }

    if (!formData.loading_point.trim() || !formData.unloading_point.trim()) {
      toast({
        title: "Eksik bilgi",
    ...
    try {
      ...
      // supplier_ref_no DB trigger ile otomatik üretilir
      const positionData = {
        customer_id: formData.customer_id,
        supplier_id: formData.supplier_id,
        loading_point: formData.loading_point.trim(),
        unloading_point: formData.unloading_point.trim(),
        ...
        sales_exchange_rate: formData.sales_exchange_rate || 1,
        cost_price: formData.cost_price || 0,
        cost_currency: formData.cost_currency,
        cost_exchange_rate: formData.cost_exchange_rate || 1,
      ...
      let description = translateSupabaseError(error);

      if (
        lowerMsg.includes("vehicle_plate") ||
        lowerMsg.includes("sales_exchange_rate") ||
        lowerMsg.includes("cost_exchange_rate") ||
        lowerMsg.includes("exchange_rates_snapshot") ||
        (lowerMsg.includes("column") && lowerMsg.includes("does not exist")) ||
        (lowerMsg.includes("could not find") && lowerMsg.includes("column"))
      ) {
        description =
          "Veritabanı şeması güncel değil. Lütfen Supabase SQL Editor'de MIGRATIONS_2026.sql ve ADD_EXCHANGE_RATES_COLUMNS.sql dosyalarını çalıştırın.";
      } else if (lowerMsg.includes("invalid input syntax for type uuid")) {
        description = "Müşteri veya tedarikçi seçimi geçersiz. Lütfen listeden seçin.";
      } else if (lowerMsg.includes("invalid input value for enum")) {
        description = "Geçersiz para birimi veya durum değeri.";
      }
