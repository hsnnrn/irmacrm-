...
export default function CreatePositionPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const sequence = Math.floor(Math.random() * 9999) + 1; // Random for demo
      const supplierTaxId = suppliers?.find(s => s.id === formData.supplier_id)?.tax_id || "0000";
      const supplierRefNo = generateSupplierRefNo(year, sequence, supplierTaxId);
      ...
      toast({
        ...
        description: `Pozisyon başarıyla kaydedildi. Referans No: ${supplierRefNo}`,
      });
